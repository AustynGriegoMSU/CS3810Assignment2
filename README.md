## Assignment 2
## Austyn Griego
## CS3810 Database Structures
## 03/30/2026

# Description:

1. Write the SQL to create tables (see the tables below) – make sure to include at least two CONSTRAINTS for each table.

2. Write the SQL to find the names of sailors with age over 20 who have not reserved a red boat. Include Sailor Names, Sailor ID, Sailor Age, and Sailor Rating in the query.

# Tables:

![SBR SQL tables](./Sailors-Boats-Reservations%20sql%20tables.png)

